  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex h-fit max-w-full gap-2 overflow-x-scroll">
        ...
        <div key={mockThumbnail.url} className="flex min-w-fit flex-col gap-1">
          <img
            src={mockThumbnail.url}
            alt="image"
            className="h-56 w-auto rounded-lg object-contain"
          />
          <p className="text-sm">
            From{" "}
            {new Date(mockThumbnail.createdAt).toLocaleDateString("en-GB", {
              day: "2-digit",
              month: "2-digit",
              year: "numeric",
            })}
          </p>
          <DownloadRecentThumbnail url={mockThumbnail.url} />
        </div>
        {/* ))} */}
      </div>
      {/* )} */}
